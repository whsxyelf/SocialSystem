				...
				$.each(userList, function(index, obj) {
					if (index >= (page * 4) && index < ((page + 1) * 4)) {
						tableStr += '<div class="user1"><div class="user1-left">' +
							...
							'<img class="concern" name="' + obj.userId + '" src="images/关注.png"></div></div>'
					}
				})
				$("#recommend-bar").html(tableStr)
				$(".concern").on("click", function(event) {
					concernedId = event.target.name
		...
		success: function(data) {
			if (data.success) {
				...
				$.each(essayList, function(index, obj) {
					...
					tableStr += '<div class="box"><div style="position: relative;" class="box-top"><img name="' + obj.essayId +
						'" class="del-btn" src="images/delete.png"/><div class="box-top-left clearfix">' +
				...
				if (tableStr != '') {
					...
					$("#box-bar").html(
						'<div class="box" style="background: url(images/empty.jpg@1280w_1l_2o_100sh.jpg);background-position: 50% 50%;">' +
		...
		success: function(data) {
			if (data.success) {
				...
				$.each(newsList, function(index, obj) {
					...
					newsStr += '<li style="cursor:pointer;"><a target="_blank" href="' + path + "news/" + obj.newsId + '">' +
						newsTitle + '</a><span></span></li>'
				})
